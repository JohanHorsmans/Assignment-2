...
clean %>% subset(VISIT == "1") %>% ggplot(aes(Diagnosis))+geom_bar()

clean %>% subset(VISIT == "1") %>% ggplot(aes(Gender))+geom_bar()

clean %>% subset(VISIT == "1") %>% ggplot(aes(Age))+geom_bar()
...
#alpha = Error
#face_wrap(separate plots)
#legend.position = none (revome individual part legend)
...
MuMIn::r.squaredGLMM(model0) #why?
...
```{r ex4, include = FALSE}
chi_model0 <- lmer(CHI_MLU ~ 1 + MOT_MLU + (1+VISIT|SUBJ), clean, REML =F)
summary(chi_model0)
...
chi_model2 <- lmer(CHI_MLU ~ VISIT*Diagnosis + MullenRawv1 + (1+VISIT|SUBJ), clean, REML =F)
summary(chi_model2)

chi_model3 <- lmer(CHI_MLU ~ VISIT*Diagnosis + MullenRawv1 + ExpressiveLangRawv1 + (1+VISIT|SUBJ), clean, REML =F)
summary(chi_model3)

chi_model4 <- lmer(CHI_MLU ~ VISIT*Diagnosis + MullenRawv1 + ExpressiveLangRawv1 + MOT_MLU +(1+VISIT|SUBJ), clean, REML =F)

summary(chi_model4)

chi_model5 <- lmer(CHI_MLU ~ VISIT*Diagnosis + MullenRawv1 + ExpressiveLangRawv1 + MOT_MLU + Socializationv1 + (1+VISIT|SUBJ), clean, REML =F)
summary(chi_model5)

chi_model6 <- lmer(CHI_MLU ~ VISIT*Diagnosis + (1+VISIT+ExpressiveLangRawv1+MullenRawv1+Socializationv1|SUBJ), clean, REML=F)
summary(chi_model6)

anova(chi_model0,chi_model1,chi_model2,chi_model3,chi_model4,chi_model5,chi_model6)

MuMIn::r.squaredGLMM(chi_model0)
MuMIn::r.squaredGLMM(chi_model1)
MuMIn::r.squaredGLMM(chi_model2)
MuMIn::r.squaredGLMM(chi_model3)
MuMIn::r.squaredGLMM(chi_model4)
MuMIn::r.squaredGLMM(chi_model5)
MuMIn::r.squaredGLMM(chi_model6)
